...
namespace SimpleAsyncClient
{
    public interface IAsyncClient
    {
        ...
        Task<TResponse> Send<TRequest, TResponse>(TRequest request, HttpMethod httpMethod, string requestUri) where TRequest : class where TResponse : class;
        Task<TResponse> Send<TResponse>(HttpMethod httpMethod, string requestUri, HttpContent content = null) where TResponse : class;
        Task<TResponse> Post<TRequest, TResponse>(TRequest request, string requestUri, string fileName, Stream stream) where TRequest : class where TResponse : class;
    }
}